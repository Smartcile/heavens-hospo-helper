...
describe('dueJobs — expiry scan', () => {
  it('is NOT due before 03:00 local time', () => {
    const state: CronState = { lastProductPullAt: Date.now(), lastOrderPullAt: Date.now(), lastExpiryScanDate: null }
  ...
  it('is NOT due again on the same local date', () => {
    const state: CronState = { lastProductPullAt: Date.now(), lastOrderPullAt: Date.now(), lastExpiryScanDate: '2026-07-16' }
    expect(dueJobs(state, UTC_1500, TZ).expiryScan).toBe(false)
  })

  it('is due again the next local day', () => {
    const state: CronState = { lastProductPullAt: Date.now(), lastOrderPullAt: Date.now(), lastExpiryScanDate: '2026-07-15' }
    expect(dueJobs(state, UTC_1500, TZ).expiryScan).toBe(true)
  })
})

describe('dueJobs — order pull', () => {
  it('is due on first run (no last-run state)', () => {
    const state: CronState = { lastProductPullAt: Date.now(), lastOrderPullAt: null, lastExpiryScanDate: null }
    expect(dueJobs(state, UTC_1400, TZ).orderPull).toBe(true)
  })

  it('is NOT due when last pull was under 15 minutes ago', () => {
    const state: CronState = {
      lastProductPullAt: Date.now(),
      lastOrderPullAt: UTC_1400.getTime() - 5 * 60 * 1000,
      lastExpiryScanDate: null,
    }
    expect(dueJobs(state, UTC_1400, TZ).orderPull).toBe(false)
  })

  it('is due exactly at the 15-minute interval', () => {
    const state: CronState = {
      lastProductPullAt: Date.now(),
      lastOrderPullAt: UTC_1400.getTime() - ORDER_PULL_INTERVAL_MS,
      lastExpiryScanDate: null,
    }
    expect(dueJobs(state, UTC_1400, TZ).orderPull).toBe(true)
  })
