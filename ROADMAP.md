...
✅ **WooCommerce Two-Way Sync** — Product webhooks (created/updated/deleted) for instant Woo→app product sync, app→Woo push (product name/price/category on save + order status changes), echo guard (`_updated_by` + 2-min window), internal cron scheduler (15-min product + order pull + daily expiry scan, no host crontab), SyncLog model + `/admin/sync` dashboard (PULL/PUSH NOW buttons, live activity feed with error visibility), order REST API pull (manual + scheduled) with shared order processor used by both webhooks and pulls
✅ **Settings** — WooCommerce section (store URL, consumer key/secret, webhook secret, active toggle with lastSync display)
✅ **Orders Dashboard** — Table sorted by fulfillmentDate, expandable line items, auto-seating status indicator, STATUS dropdown (pushes to WooCommerce), FOH VIEW tab (date picker, booking cards with table assignments + dietary badges, category totals sidebar)
✅ **Allergen System** — 23 allergen toggle buttons (Almond through Wheat) on recipe editor LINK TO MENU section, stored as comma-separated `dietaryInfo` on MenuItem, surfaced in FOH and Kitchen views with gold `[#c4a530]` badges
✅ **Kitchen Worker View** — `/w/kitchen` auto-refreshing page (15s) showing today's order items grouped by table, PREP TOTALS grid with dietary info, unassigned items section. `KitchenStatus` enum (PENDING/COOKING/READY/SERVED) groundwork on `WooOrderItem` for future live service mode
☐ **Live Service Mode** — Kitchen status workflow (PENDING → COOKING → READY → SERVED) synchronized across worker devices, mark orders as cooking/served from the kitchen view
